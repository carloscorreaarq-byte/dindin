...
- `2026-05-01`: acelerada a experiencia de leitura do app: o `Resumo` passou a renderizar imediatamente do cache local e atualizar em paralelo com timeouts mais curtos, e a abertura de `Gastos`/`Entradas` ganhou rota rapida com cache imediato, consultas paralelas ao legado e a `lancamentos`, e limite reduzido por carga; o cache do app foi atualizado para `financas-v19`
- `2026-05-02`: reforcada a estabilidade de sessao e reduzida a pressao de leitura: `ensureActiveSession()` passou a revalidar/renovar a sessao periodicamente, a limpeza de sessao agora acontece explicitamente quando o auth state cai, e o `Resumo` deixou de disparar consultas legadas extras sem necessidade, usando menos chamadas concorrentes; o cache do app foi atualizado para `financas-v20`
...
- `2026-05-02`: iniciada a fase de estabilizacao estrutural do app: a hidratacao de sessao deixou de duplicar carga no bootstrap, `Resumo` e listas ganharam travas para evitar consultas sobrepostas, o caminho antigo de lista foi despriorizado em favor do fluxo rapido unico, e o projeto recebeu o script `financas-fase-estabilizacao.sql` com indices para o hot path de `lancamentos` e parcelamentos; o cache do app foi atualizado para `financas-v25`
